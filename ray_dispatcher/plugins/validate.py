...
def validate_worker_py(
    ...
    *,
    ...
    """Validate a staging ``worker.py`` for ``plugin_id``."""
    ...
    if target.name != "worker.py":
        errors.append(f"plugin file must be named worker.py, got {target.name!r}")
    ...
    if run_discover and not errors:
        ...
        if discovered["handler_ids"]:
            handler_ids = discovered["handler_ids"]
        ...
        for handler_id in handler_ids:
            ...
            if not handler_id.startswith(prefix):
                errors.append(
                    f"handler_id {handler_id!r} must start with {prefix!r}"
                )
    ...
    for node in tree.body:
        ...
        for target in node.targets:
            ...
            if target.id == "HANDLERS":
                has_handlers = True
                parsed = _parse_handlers_assign(node.value, plugin_id)
...
def _parse_handlers_assign(node: ast.AST, plugin_id: str) -> dict[str, Any]:
    errors: list[str] = []
    warnings: list[str] = []
    handler_ids: list[str] = []
    ...
    for index, elt in enumerate(node.elts):
        ...
        handler_id = mapping.get("handler_id")
        if handler_id is None:
            errors.append(f"HANDLERS[{index}] must set explicit handler_id")
            continue
        if not handler_id.startswith(prefix):
            errors.append(
                f"HANDLERS[{index}] handler_id {handler_id!r} must start with {prefix!r}"
            )
        handler_ids.append(handler_id)
        if "entrypoint" not in mapping:
